...
from teplo500.salus import salus_alert
from teplo500.salus import salus_device
from teplo500.salus import salus_emul
from teplo500.salus import salus_history_helper
...
def CreateAndLoad(client_id):
	...
	log_debug('CreateAndLoad: creating client '+str(client_id)+'...')
	...
	log_debug('CreateAndLoad: created client '+str(client_id))
...
class SalusClient:
	...
	def get_auto_update(self):		
		if 'auto_update' in self.config:
			return int(self.config['auto_update'])
		else:
			return int(get_app().config['defaults']['auto_update'])
	...
	def get_auto_upday_txt(self):
		...
		if period<3600:
			return str(round(period / 60))+' '+locstr('minute(s)/timediff')
		elif period<86400:
			return str(round(period / 3600))+' '+locstr('hour(s)/timediff')
		else:
			return str(round(period / 86400))+' '+locstr('day(s)/timediff')
	...
	def run_alerts(self):
		...
		for id in salus_alert.ALERT_IDS:
			...
			alert = salus_alert.CreateAlert(id,conf,existing_data)
			if alert is None:
	...
	def load(self):
		...
		if data is not None:
			...
			for device_data in data['devices']:
				device = salus_device.SalusDevice(self) 
	...
	def save_history(self):
		return salus_history_helper.save_client_history(self)
	...
	def _update_device_list_from_site(self):
		...
		if get_app().salus.is_real_mode():
			...
			devices_html = salus_emul.emul_load_devices()
		else:
	...
	def _parse_html_devices(self,devices_html):
		...
		for input_node in inputs_nodes:
			...
			if not dev:
				dev = salus_device.SalusDevice(self,id)		
