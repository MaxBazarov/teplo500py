...
def home_page_register(flask):
    ...
    flask.add_url_rule('/home/zones/<zone_id>/temp/edit', 'homepage_edit_zone_temp', homepage_edit_zone_temp)
    flask.add_url_rule('/home/zones/<zone_id>/temp/save', 'homepage_save_zone_temp', homepage_save_zone_temp,methods=['POST'])
...
def homepage_edit_zone_temp(zone_id):
    page = HomePage()    
    page.zone_id = zone_id

    return core.app.create_response( page.show_edit_zone_temp())

def homepage_save_zone_temp(zone_id):
    page = HomePage()
    page.zone_id = zone_id

    return core.app.create_response( page.do_save_zone_temp() )
...
class HomePage(AbstractPage):        
    ...
    def show_edit_zone_temp(self,zone=None):
        ...
        vars = {
            ...
            'ok_msg' : self.ok_msg,
            'temps':temps,
            'form_url' : core.app.urls['HOME_SUBURL']+'/zones/'+str(zone.id)+"/temp/save",
            'man_temp' : zone.current_mode_temp if (zone.mode==salus_zone.MODE_AUTO or zone.mode==salus_zone.MODE_MAN) else zone.current_temp,
            'name':zone.name   
        }

        return self.compile_page('home_edit_zone_temp.tmpl',vars)
    ...
    def do_save_zone_temp(self):
        while(True):
            ...
            ## read submitted data
            new_man_temp = int(http_post_param('man_temp').lstrip().rstrip())
            if new_man_temp=='':
    ...
    def do_update(self):
        ...
        self.ok_msg = locstr('Data succesfully updated.')
