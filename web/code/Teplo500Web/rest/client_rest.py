...
from teplo500web.rest.abstract_rest import AbstractREST
...
class ClientREST(AbstractREST):
    ...
    def get_client_data(self,sdate):
        ...
        found = salus_history_helper.find_client_history(app.client,odate)
        if not found:
            log_debug('ClientREST.get_client_data(): find_client_history failed')
            return self.ShowNoData()

        raw_data = salus_history_helper.load_client_history(app.client,odate)
        if raw_data is None:
